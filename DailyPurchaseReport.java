...
public class DailyPurchaseReport extends JFrame implements ActionListener {
    ...
    public DailyPurchaseReport() {
        ...
        txt1.setToolTipText("EnterPurchaseReportDate");
        jframe.add(txt1);
       ...
       model.addColumn("AMOUNT");
        ...
        jframe.setTitle("Daily Purchase Report");
        ...
        if (ae.getSource() == submit) {//list
            ...
            try {
                if (((txt1.getText()).equals(""))) {
                    ...
                    rs = statement.executeQuery("SELECT Iamount,Iname,Ipurdate,Itype,Ipurprice,Sname from purchasereport where Ipurdate='" + txt1.getText() + "' ");
                    while (rs.next()) {
                        model.insertRow(r++, new Object[]{rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6)});
                        foundrec = 1;
                    }
                    if (foundrec == 0) {
                        JOptionPane.showMessageDialog(null, "Not any Item purchase on given date");
                    }
                }
                connection.close();
